...
def main():
    ...
    for idx, z_val in enumerate(z_slices):
        ...
        ax.imshow(masked, cmap='hot', origin='lower', extent=[0, size, 0, size])
    ...
    fig.suptitle('GPU-VDB Metaballs 2D (Blue=Root, Green=Internal, Red=Leaf)', fontsize=14, weight='bold')
    plt.tight_layout()
    plt.savefig(Path(__file__).parent / 'metaballs_2d.png', dpi=150, bbox_inches='tight')
    print("Saved metaballs_2d.png")
    ...
    sc = ax.scatter(coords_active[:, 0], coords_active[:, 1], coords_active[:, 2],
                   c=vals_active, cmap='hot', s=3, alpha=0.6)
    plt.colorbar(sc, ax=ax, label='Value', shrink=0.5)
    
    ax.set_xlabel('X')
    ax.set_ylabel('Y')
    ax.set_zlabel('Z')
    ax.set_title(f'GPU-VDB Metaballs 3D ({len(coords_active)} voxels)', fontsize=14, weight='bold')
    ax.set_xlim(0, 128)
    ax.set_ylim(0, 128)
    ax.set_zlim(0, 128)

    plt.savefig(Path(__file__).parent / 'metaballs_3d.png', dpi=150, bbox_inches='tight')
    print("Saved metaballs_3d.png")
